...
func PostRegister(w http.ResponseWriter, r *http.Request) {
	...
	users := strings.Split(configDb, ":")
	pw := strings.Split(users[1], "@")
	port := strings.Split(users[2], "/")
	msgArgs := fmt.Sprintf("host=%s port=%s user=%s password=%s dbname=%s sslmode=%s connect_timeout=%s", pw[1], port[0], users[0], pw[0], port[1], "disable", "5")
